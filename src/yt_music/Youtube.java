...
public class Youtube {
	...
	public static ArrayList<Audio> getRelated(String videoID, int numberOfItems) {
		try {
	...
	private static ArrayList<Audio> parseAPIJson(URL url) throws IOException, JSONException {
		...
		for (int i = 0; i < items.length(); i++) {
			...
			iconURL = vidItem.getJSONObject("snippet").getJSONObject("thumbnails").getJSONObject("medium")
					.getString("url");
	...
	private static URL getRelatedURL(String videoID, int numberOfItems) throws MalformedURLException {
		if (videoID.length() != 11) {
			System.err.println("Wrong URL length");
			throw new MalformedURLException();
		}
		try {
			return new URL(YOUTUBE_RELATED.replace(":NUMBER_OF_ITEMS:", String.valueOf(numberOfItems))
	...
	private static URL getSearchUrl(String search, int numberOfItems) {
		try {
			return new URL(YOUTUBE_SEARCH_URL.replace(":NUMBER_OF_ITEMS:", String.valueOf(numberOfItems))
